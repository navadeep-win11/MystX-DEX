# MystX DEX — Termux Fork + Custom Web GUI

**MystX DEX** is a next-generation developer terminal and local web workspace built on the open-source Termux core engine. It combines the full command-line power of Termux with a sleek, local-first DSH-Web-style Web GUI accessible via browser and embedded Android WebView.

---

## Quickstart

Inside the MystX DEX terminal, type:

```bash
mystx
```

This starts the local MystX DEX Web server (`http://127.0.0.1:8888`) and opens the Web GUI.

### CLI Commands

- `mystx`: Start the Web GUI server and open the interface.
- `mystx start`: Start the local Web GUI server daemon in background.
- `mystx stop`: Stop the local Web GUI server cleanly.
- `mystx restart`: Restart the server.
- `mystx status`: Check running status, PID, and local URL.
- `mystx help`: Display usage help.

---

## Features

- **Full Termux Terminal Runtime**: Shell, apt/pkg package management, filesystems, and environment variables completely preserved.
- **Custom DSH-Web-Style GUI**: Obsidian-dark mobile-first web interface with real-time bidirectional WebSocket PTY streaming.
- **Local-First & Offline**: Zero cloud dependencies, zero telemetry, localhost-only binding.
- **Multi-Session Manager**: Create, switch, and kill terminal sessions seamlessly.
- **File Explorer**: Browse local home and storage files, with built-in viewer.
- **Extensible AI Architecture**: Extensible agent workspace ready for local models (Ollama, Llama.cpp) and cloud APIs.
- **Integrated Android WebView**: Convenient in-app launcher activity (`MystxWebActivity`) with quick toggle back to the terminal.

---

## Upstream & Open Source Notice

MystX DEX is a fork of the open-source [Termux](https://termux.dev) project. The core terminal engine is licensed under GPLv3. All upstream credits and licenses are respectfully preserved.
